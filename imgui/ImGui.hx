...
class ImGui
{
    ...
    public static function shortcutColored(col : ImVec4, txt : String, shortcut : String) { 
		textColored(col, txt); 
		var availSize : ImVec2 = getContentRegionAvail();
		var textSize : ImVec2 = calcTextSize(shortcut);
		sameLine(availSize.x - textSize.x); 
	...
	public static function comboWithArrows<T>(?label : String, current_item : Int, items : Array<T>, get_name : Int -> String, changed : Int -> Void) {
		ImGui.pushID(label == null ? "###comboWithArrows" : label);
		var style : ImGuiStyle = ImGui.getStyle();
		var w = label != null ? ImGui.calcItemWidth() : ImGui.getContentRegionAvail().x;
