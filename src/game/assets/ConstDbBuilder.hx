					...
					for (v in l.values)
					{
						...
						if (v.subValues != null && Reflect.fields(v.subValues).length > 0)
						{
							...
							for (k in Reflect.fields(v.subValues))
							{
								...
								var const : Constant = !subValueTypes.exists(k) ? CFloat(Std.string(rawVal)) : switch subValueTypes.get(k)
									.typeStr {
									...
									case _:
										Context.fatalError("Unexpected CastleDB typeStr " + subValueTypes.get(k)
											.typeStr + " for sub-value init expr", pos);
								}
								initers.push({
									field: k,
