...
class Entity
{
	...
	inline function get_camera() return game.camera;
	...
	{
		return
			interpolateSprPos ? M.lerp(lastFixedUpdateX, (cx + xr) * Const.GRID, game.getFixedUpdateAccuRatio()) : (cx + xr) * Const.GRID;
	}
	...
	{
		return
			interpolateSprPos ? M.lerp(lastFixedUpdateY, (cy + yr) * Const.GRID, game.getFixedUpdateAccuRatio()) : (cy + yr) * Const.GRID;
	}
	...
	inline function get_centerX() return attachX + (0.5 - pivotX) * wid;
	/** Center Y pixel coordinate of the bounding box **/
	public var centerY(get, never) : Float;
	inline function get_centerY() return attachY + (0.5 - pivotY) * hei;
	...
	**/
	...
	public inline function debug(?v : Dynamic, c : Col = 0xffffff)
	{
		...
		if (v != null)
		{
			if (debugLabel == null)
			{
				debugLabel = new h2d.Text(Assets.font(Regular), Game.ME.scroller);
